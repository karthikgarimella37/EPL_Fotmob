...
SELECT
    ...
    ht.TeamColor AS HomeTeamColor,
    
    -- Away Team Info
    m.AwayTeamID,
    at.TeamName AS AwayTeamName,
    at.ImageUrl AS AwayTeamImageUrl,
    at.TeamColor AS AwayTeamColor,
...
SELECT
    ...
    COALESCE(SUM(ps.ChancesCreated), 0) AS TotalChancesCreated,
    COALESCE(SUM(ps.SuccessfulDribbles), 0) AS TotalSuccessfulDribbles,
    COALESCE(SUM(ps.PassesintoFinalThird), 0) AS TotalPassesIntoFinalThird,
    COALESCE(SUM(ps.TouchesinOppBox), 0) AS TotalTouchesInOppBox,
    COALESCE(SUM(ps.Dispossessed), 0) AS TotalDispossessed,
    COALESCE(SUM(ps.TacklesWon), 0) AS TotalTacklesWon,
    COALESCE(SUM(ps.Interceptions), 0) AS TotalInterceptions,
    COALESCE(SUM(ps.Clearances), 0) AS TotalClearances,
    COALESCE(SUM(ps.HeadedClearances), 0) AS TotalHeadedClearances,
    COALESCE(SUM(ps.DefensiveActions), 0) AS TotalDefensiveActions,
    COALESCE(SUM(ps.Recoveries), 0) AS TotalRecoveries,
    COALESCE(SUM(ps.DribbledPast), 0) AS TotalDribbledPast,
    COALESCE(SUM(ps.DuelsWon), 0) AS TotalDuelsWon,
    COALESCE(SUM(ps.DuelsLost), 0) AS TotalDuelsLost,
    COALESCE(SUM(ps.GroundDuelsWon), 0) AS TotalGroundDuelsWon,
    COALESCE(SUM(ps.AerialDuelsWon), 0) AS TotalAerialDuelsWon,
    COALESCE(SUM(ps.WasFouled), 0) AS TotalWasFouled,
    COALESCE(SUM(ps.FoulsCommitted), 0) AS TotalFoulsCommitted,
    COALESCE(AVG(ps.FotmobRating), 0) AS AvgRating,
